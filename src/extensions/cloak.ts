namespace CloakExtension {
  let isMarked = false;

  function transformDataOnImport(json: any) {
  ...
  function transformDataOnExport(json: any) {
    json.worlds.forEach(world => {
  ...
  function init(handle: FileSystemFileHandle, initialJSON: Object) {
    let root = createRoot();
  ...
  function createRoot(): Widgets.Struct {
